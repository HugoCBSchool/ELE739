...
-- Last update : Sun Mar 14 13:48:27 2021
...
entity Control is
	...
	port(
		...
		o_gen_act		:	out std_logic;	--! control bit towards signal generator
		o_fil_act	    :	out std_logic;	--! control bit towards signal filter
		o_reset	  		:	out std_logic;	--! reset bit towards signal generator and filter
		
		o_led_idle      :	out std_logic; --! led output for idle mode 
...
begin
	...
	o_gen_act<= '1' when cstate=filter else
	            '1' when cstate=sig_gen else
	            '0';
	o_fil_act<= '1' when cstate=filter else '0';
	
	
	o_gen_led<= cstate(0);
	o_fil_led<= cstate(1);
	o_led_idle<= not (cstate(0) or cstate(1)) ;
	...
	----------------
	----------MUX-------------------------------
	process(i_bus_g, i_bus_f, s_mode ) is
    begin
 
        case s_mode is
           
            when sig_gen =>            -- use  bit to check state and select
                o_bus <= i_bus_g;
            when filter =>
                o_bus <= i_bus_f;
            when others =>
